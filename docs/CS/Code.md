...
- [algorithm](#algorithm)
  ...
  - [位图（集合）](#位图集合)
  - [位运算实现加减乘除](#位运算实现加减乘除)
  - [链表高频题和必备技巧](#链表高频题和必备技巧)
    - [返回两个无环链表相交的第一个结点](#返回两个无环链表相交的第一个结点)
    - [每k个结点一组翻转链表](#每k个结点一组翻转链表)
    - [复制带随机指针的链表](#复制带随机指针的链表)
    - [判断链表是不是回文结构](#判断链表是不是回文结构)
    - [链表第一个入环结点](#链表第一个入环结点)
    - [在链表上排序。时间复杂度O(n * log n)，空间复杂度O(1)，有稳定性](#在链表上排序时间复杂度on--log-n空间复杂度o1有稳定性)
  - [数据结构设计高频题](#数据结构设计高频题)
    - [setAll功能哈希表](#setall功能哈希表)
    - [实现LRU结构](#实现lru结构)
    - [插入、删除和获取随机元素O(1)时间的结构](#插入-删除和获取随机元素o1时间的结构)
    - [插入、删除和获取随机元素O(1)时间且允许有重复数字的结构](#插入-删除和获取随机元素o1时间且允许有重复数字的结构)
    - [快速获得数据流的中位数的结构](#快速获得数据流的中位数的结构)
    - [最大频率栈](#最大频率栈)
    - [全O(1)的数据结构](#全o1的数据结构)
